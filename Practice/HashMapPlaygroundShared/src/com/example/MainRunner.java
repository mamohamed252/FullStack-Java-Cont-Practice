...
//		hMap.put(personA.getId(), personA);
//		printHashMap(hMap);
...
//		});
		...
		int totalSalary = 0;
		for(Entry<Integer, Person> element : hMap.entrySet()) {
			totalSalary += element.getValue().getSalary();
			System.out.printf("[KEY: %-10d VALUE: %-10s]%n%n", element.getKey(), element.getValue());
		}
	...
	public static void contains(Map<Integer, Person> hMap, int key) {
		...
		if (flag)
			System.out.printf("KEY %d FOUND IN MAP."
					+ "VALUE: %s%n", key, hMap.get(key).toString()); // get(key) returns value associated with key
		else
